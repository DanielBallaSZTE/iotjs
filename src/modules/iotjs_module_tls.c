...
JS_FUNCTION(Write) {
  ...
  size_t length = iotjs_bufferwrap_length(buf);
  ...
  jbuffer = iotjs_bufferwrap_create_buffer(pending);
  buf = iotjs_bufferwrap_from_jbuffer(jbuffer);
...
static void tls_handshake(iotjs_tls_t *tls_data, jerry_value_t jthis) {
  ...
  if (ret_val != 0) {
    ...
    tls_data->handshake_state = SSL_HANDSHAKE_ERROR;
    error = true;
    authorized = false;
  } else {
  ...
  iotjs_jargs_append_jval(&jargv, jerry_create_boolean(error));
  iotjs_jargs_append_jval(&jargv, jerry_create_boolean(authorized));
...
JS_FUNCTION(Read) {
  ...
  if (jerry_value_to_boolean(jargv[0])) {
    jerry_value_t jbuffer = JS_GET_ARG(0, object);
  ...
  do {
    ...
    if (tls_data->handshake_state != SSL_HANDSHAKE_DONE) {
      if (tls_data->handshake_state != SSL_HANDSHAKE_ERROR) {
        tls_handshake(tls_data, jthis);
      }

      if (tls_data->handshake_state != SSL_HANDSHAKE_DONE) {
        bool result = (tls_data->handshake_state != SSL_HANDSHAKE_ERROR);
    ...
    while (true) {
      ...
      if (ret_val == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY) {
        jerry_value_t fn =
            iotjs_jval_get_property(jthis, IOTJS_MAGIC_STRING_END);
...
jerry_value_t InitTls() {
  ...
  iotjs_jval_set_method(jtls, IOTJS_MAGIC_STRING_END, End);
