...
JS_FUNCTION(TlsConstructor) {
  ...
  if ((ret = mbedtls_ssl_config_defaults(&tls_data->conf,
  ...
  mbedtls_ssl_set_bio(&tls_data->ssl, &tls_data->ssl_bio, mbedtls_net_send,
  ...
  mbedtls_ssl_set_bio(&tls_data->ssl,
                      &tls_data->ssl_bio,
                      iotjs_bio_net_send,
                      iotjs_bio_net_recv,
                      NULL);
...
JS_FUNCTION(Write) {
  ...
  size_t ret_val = (size_t)mbedtls_ssl_write(&tls_data->ssl,
...
jerry_value_t InitTls() {
  ...
  iotjs_jval_set_property_jval(jtls, IOTJS_MAGIC_STRING_TLSWRAP, tlsConstructor);
